// By Alexandre Junkes Pinotti
// based on the algorithm of Miles Shang <mail@mshang.ca>
// MIT license
...
var layersCount = 0;
var queue_factor_layers = new Array();
...
Node.prototype.pushFactor = function() {
	if(checkLayer(this.layerLevel) == false) {
		queue_factor_layers.push(this.layerLevel);
	}
}

function checkLayer(x) {
	for(var i=0; i < queue_factor_layers.length; i++) {
		if(queue_factor_layers[i] == x) {
			//this factor layer has been inserted already
			return true;
		}
	}
	return false;
...
Node.prototype.check_factor = function() {
	if(this.value == "-1" || this.value == "X" )
		this.isFactor = true;
}

Node.prototype.set_factor = function(){
	this.isFactor = true;
}
...
Node.prototype.set_siblings = function(parent) {
	...
	//check if this value corresponds to a factor
	this.check_factor();
	if(this.isFactor == true) {
		this.pushFactor();
	}
...
Node.prototype.setLayer_Factor = function(parent){
	for (var i = 0; i < this.children.length; i++) {
		this.children[i].setLayer_Factor(this);
	}
	if(checkLayer(this.layerLevel) == true)
		this.set_factor();
}
...
function go(str, font_size, term_font, nonterm_font, vert_space, hor_space, color, term_lines, isImage) {	
	// Clean up the string
	str = str.replace(/^\s+/, "");
	var open = 0;
	...
	//init variables
	layersCount = 0;
	queue_factor_layers.length = 0
	//
	root.set_siblings(null);
	root.setLayer_Factor(null);
	root.check_triangle();
	...
	// Swap out the image
	//true: return the image
	//false: return the string
	return Canvas2Image.saveAsPNG(canvas, isImage);
}
